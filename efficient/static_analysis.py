...
class MPCPlanner:
    ...
    def find_array_access_targets(self, func_addr):
        """
        Smart planner: tracks array access instructions only if they 
        lack bounds checks (filters out false milestones in _good branches)       
        """
       ...
       # Iterating through all basic blocks inside the function
        for block in func.blocks:
            # Checking for comparison or validation instructions within the current block
            has_bounds_check = False
            for insn in block.capstone.insns:
                # Searching for comparison instructions generated by the compiler for "if (index < size)"
                if insn.mnemonic in ['cmp', 'test']:
                    has_bounds_check = True
                    break

           # If a bounds check is detected in the block, its array instructions 
           # are completely ignored to avoid path generation toward safe Good Sinks 
            if has_bounds_check:
                continue

            # If the block is unverified (lacks checks), 
            # we parse it for memory access instructions
            for insn in block.capstone.insns:
                op_str = insn.op_str
                if "[" in op_str and ("*" in op_str or "+" in op_str):
                        # Additional filter: omitting architectural stack frame accesses (ebp/esp)                    if "ebp" not in op_str and "esp" not in op_str:
